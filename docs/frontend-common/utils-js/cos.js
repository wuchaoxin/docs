...
export function uploadFile(file, type = 'img', handle, uploadFileParams) {
    if (!file.size)
        return Promise.reject('未选择上传文件');
...
export function uploadMultipleFiles(fileList, type = 'img', handle, uploadFilesParams) {
...
export function removeFile(fileName, type = 'img', handle, deleteObjectParams) {
    return new Promise((resolve, reject) => {
        const { bucket, region, key } = getInfo(type);
        initCOS(false)
            .then(() => {
...
export function removeMultipleFiles(fileNameList, type, handle, deleteMultipleObjectParams) {
    return new Promise((resolve, reject) => {
        const { bucket, region, key } = getInfo(type);
        const objects = (() => {
            const result = [];
 ...
 */
// #endregion createDir
export function createDir(dir, type = 'img') {
...
export function getDirFiles(dir, type = 'img', getBucketParams) {
    return new Promise((resolved, rejected) => {
...
function replaceQcloudDomain(url) {
