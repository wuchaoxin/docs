...
import { BaseConfig, Event, PageConfig, Queue } from './types/typings/buriedPoint'
...
export function initBuriedPoint(baseConfig: BaseConfig, pageConfig: PageConfig, event: Event) {
  if (window[YM_BURIED_POINT]) {
    return false
  } else {
    const buriedPoint = new BuriedPoint(event) as BuriedPoint
...
export function sensorsTrack(eventName: string, eventMap: AnyObject) {
  if (_checkBuriedPoint()) {
    const buriedPoint = window[YM_BURIED_POINT] as BuriedPoint
...
export function pageViewTrack(eventName: string) {
  if (_checkBuriedPoint()) {
    const buriedPoint = window[YM_BURIED_POINT] as BuriedPoint
...
export function getAppKeyAndSecret(type: 'YM' | 'YMGJ') {
  const env = getEnv()
...
class BuriedPoint {
  ...
  init(baseConfig: BaseConfig, pageConfig: PageConfig) {
    this._loadScript()
      .then(() => {
   ...
   */
  sensorsTrack(eventName: string, eventMap: AnyObject) {
   ...
   */
  pageViewTrack(eventName: string) {
    if (this._checkScript(false)) {
      const sensors = window[YM_BURIED_POINT_SENSORS]
  ...
  _initSensors(pageConfig: PageConfig) {
  ...
  _initBuriedPoint(baseConfig: BaseConfig) {
    const config = {
  ...
  _loadScript() {
    ...
    for (const src of Object.values(SCRIPT_MAP)) {
      ...
      promiseList.push(
        new Promise<void>((resolevd, rejected) => {
