...
import { ReAxiosPromise } from './types/typings/request'
import { Config, ShareConfig } from './types/typings/weChat'
...
export function injectJSSDK() {
  ...
  return new Promise<void>((resolevd, rejected) => {
 ...
 */
// #endregion waitJSSDK
export function waitJSSDK(fn: () => void) {
  if (isHaveJSSDK()) {
    fn()
  } else {
    _queue.push(fn)
...
export function initJSSDK() {
  ...
  const handleConfig = defaultLocalStorage.get(WECHAT_CONFIG_STORAGE_KEY, 'Object') as unknown as Config
  if (Object.keys(handleConfig).length > 0 && handleConfig.url === url) {
    JSSDKConfig(handleConfig)
  } else {
    requestJSSdkConfig((config: Config) => {
      JSSDKConfig(config)
    })
  }
}
function requestJSSdkConfig(callback: (config: Config) => void) {
  // 清除缓存
...
function JSSDKConfig(config: Config) {
  // 调取微信api
  window.wx.config({
    debug: false,
    appId: config.appId,
  ...
  // 微信配置失败
  window.wx.error(function (res: Dynamic) {
 ...
 */
// #endregion setShare
export function setShare(config: ShareConfig) {
...
function getJsSdkConfig(params: { url: string }): ReAxiosPromise<Data<Config>> {
  return request({
